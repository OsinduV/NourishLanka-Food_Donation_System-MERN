...
export default function UpdateStatus() {
    ...
    try {
      const fetchFoodRequests = async () => {
        const res = await fetch(
          `/api/foodrequest/getfoodrequests?foodrequestId=${foodrequestId}`
        );
  ...
  const handleSubmit = async (e) => {
    ...
    try {
      const res = await fetch(
        `/api/foodrequest/updatefoodrequeststatus/${formData._id}/${currentUser._id}`,
        {
          method: "PUT",
          headers: {
  ...
  return (
    <div className="relative flex flex-col gap-6 p-28 px-3 max-w-6xl mx-auto">
      <div className="container mx-auto">
        ...
        <div className="text-center top-0 left-0 bg-red-500 text-white px-3 py-1 rounded-full font-bold">
          Current Status -{formData.status}
        </div>
        <h1 className="text-3xl mt-3 p-3 text-center font-semibold max-w-2xl mx-auto lg:text-2xl">
          Food Request ID - {formData.foodrequestID}
        </h1>
