...
export default function VolunteerFour() {
  ...
  const schedulesByCategory = {
    "Packing and Sorting": userSchedules.filter(
      (schedule) => schedule.category === "Packing and Sorting"
    ),
    "Cooking and Preparing": userSchedules.filter(
      (schedule) => schedule.category === "Cooking and Preparing"
    ),
    "Food Distribution": userSchedules.filter(
      (schedule) => schedule.category === "Food Distribution"
    ),
  ...
  const handleShowMore = async () => {
    ...
    try {
      const res = await fetch(
        `/api/schedules/getschedules?startIndex=${startIndex}`
      );
      const data = await res.json();
      if (res.ok) {
  ...
  return (
    <div className="table-auto overflow-x-scroll md:mx-auto p-3 scrollbar scrollbar-track-slate-100 scrollbar-thumb-slate-300 dark:scrollbar-track-slate-700 dark:scrollbar-thumb-slate-500">
      <div>
        ...
        <div className="p-4 bg-gray-100 rounded-lg mb-4"></div>
                  ...
                  <Table.Row className="bg-white dark:border-gray-700 dark:bg-gray-800">
                    ...
                    <Table.Cell>
                      <Link
                        ...
                        to={`/volunteer-five/${schedules._id}/${foodbankId}`}
                      >
                        <span>Register</span>
                      </Link>
                    </Table.Cell>
