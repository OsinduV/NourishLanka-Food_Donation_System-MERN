//event organiser dashboard of all completed donation events
...
export default function ComAllDonations() {
    ...
    useEffect(() => {
      const fetchDonations = async () => {
        try {
         ...
         const res = await fetch(`/api/donation/getdonations?status=completed`);
    ...
    return (
      <div className='table-auto overflow-x-scroll md:mx-auto p-3 scrollbar scrollbar-track-slate-100 scrollbar-thumb-slate-300 dark:scrollbar-track-slate-700 dark:scrollbar-thumb-slate-500'>
             {/*if the user is event orgniser and if the requests are more than 0 , then display the requests and if not just display a message no requests yet */}
              ...
              <h2 className="text-3xl font-semibold flex">Completed Donation Requests</h2>
