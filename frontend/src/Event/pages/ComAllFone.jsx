//event organiser dashboard of all oneday completed fooddrive events
...
export default function ComAllFone() {
    ...
    useEffect(() => {
      const fetchFooddrives = async () => {
        try {
         ...
         const res = await fetch(`/api/fooddrive/getfooddrives?type=onedaydrive&status=completed`);
  ...
  return (
    <div className='table-auto overflow-x-scroll md:mx-auto p-3 scrollbar scrollbar-track-slate-100 scrollbar-thumb-slate-300 dark:scrollbar-track-slate-700 dark:scrollbar-thumb-slate-500'>
        <div className="flex items-center mb-10 justify-center mt-10 bg-white shadow-md dark:border-gray-700 dark:bg-gray-800">
            <h2 className="text-3xl font-semibold flex">One Day FoodDrive Request Lis - Completed</h2>
        </div>
                  {/* Header-like section */}
            ...
            <h2 className="text-xl font-semibold"></h2>
                ...
                <Link to='/dashboard?tab=fapprovedone'>Approved</Link>
                <Link to='/dashboard?tab=fdeclinedone'>Declined</Link>
